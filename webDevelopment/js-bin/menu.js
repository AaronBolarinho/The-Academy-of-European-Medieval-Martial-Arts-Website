...
function TPopMenu(label,icon,clickType,clickParam,status)
{
	...
	switch(clickType)
	{
		...
		case 'f':
	  	...
	  	case 'address':
			this._eClick = "_openURL('"+clickParam+"', 'new window')";
			break;
		case 'a':
		...
		case 'a_fr':
			this._eClick = "_openURL('"+clickParam+"', 'mainFrame')";
		break;
	...
	if(this._itemIndex > -1)
	{
	  var isExpandIcon = false;
		for(var i=0;i<=this._itemIndex;i++)
		{
			...
			//en produkte und service fix in der breite vergößern - alex
			if(this._items[i]._id == '_ec-43ID'){ 
				result1 += '<td nowrap style=\"width:110px\" class="TMenuItem'+this._index+'" id="pr_'+this._items[i]._id+'"';
			}
			else{
				result1 += '<td nowrap style=\"width:77px\" class="TMenuItem'+this._index+'" id="pr_'+this._items[i]._id+'"';
			}
	  ...
	  // added because of table problem 
	  // with products & service length
...
function DrawPopMenu(level)
{
	...
	if(this._itemIndex > -1)
	{
		...
		/*
		* alex - Abfrage um die Breite von bestimmten Pop-Up-Fenstern zu ändern - damit kein Umbruch
		*/
	
		if((this._id == "_ec-31ID")) { // netzanbindung und anlagensteuerung
			result += '<div id="'+this._id+'" style="filter:alpha(opacity='+this._parent._pop._alpha+'); border: 1px solid rgb(0, 0, 0); padding: 2px; position: absolute; visibility: hidden; width: 250px; background-color: whitesmoke; background-image: url(img/xp.gif); background-position: left top; background-repeat: repeat-y; display: block;">';
...
//alert ("the value for height = " + dHeight + ", and the value for length i= " + dWidth);
  if(!dWidth)
...
function _openURL(address, target) // revised target (dcvet)
{
	if(target != '') 
		{
		if(target == 'mainFrame')
			{
			parent.mainFrame.location.href = address; // Mainframe = name of target frame - default frame name
			}
		else
			{
			var newWindow = window.open(address);
			newWindow.focus();  // _blank = opens URL in a new window
			}
		}
	else {
