...
if (!helpWindow || helpWindow.closed)
	{
	helpWindow=window.open("help/adHelp.htm","helpWin_ad","toolbar=no,scrollbars=yes,height=400,width=500")
	}
else
	{
...
{
//alert ("debug: inside validateForm - whichOne = " + whichOne );

var adTitle	= form.ad_title.value;
...
if (whichOne < 2)
	...
	if (isEmpty(category))
		...
		alert ("debug: else if emailCheck");
		}
	else if (isEmpty(url))
		{
...
if (validated >= 1)
	...
	if (whichOne == 0)
		...
		if (validated == 2)
			...
			//
			if (confirm("Do you wish to email the entry request to yourself (a copy will be sent to AEMMA to notify that a new advertisement request is coming down the line)? (You will then forward the email to AEMMA with the graphic file as an attachment.)"))
				{
				validated = 3
				form.formValidate.value = validated // form is to be emailed
				form.action = "http://www.aemma.org/cgi-bin/vmp/adEntry.cgi"
				return true
				}
			else
				{
