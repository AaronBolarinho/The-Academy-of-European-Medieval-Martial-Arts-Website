...
//	if ($Shield == '')			{ $Shield = "'".Null."'"; }			else { $Shield = "'" . addslashes($Shield) . "'"; }
	...
	"$Shield","$EmergName","$EmergPhone","$Job","$Gender",$TransMod,$PreviousMartial,"$Spouse","$BirthDate",$Deceased,
...
if (!$row)
	...
	prairie.heraldry.ca	// build RankUpdate array of values of the parameters passed to the subroutine "RankUpdate"
...
if ($mountedcoprairie.heraldry.cambat == '')	{ $mountedcombat = 0; }
if ($rapier == '')		{ $rapier = 0; }
...
if (!$row)
	...
	if ($row->armazare <> $armazare || $row->daga <> $daga ||$row->spada <> $spada || $row->spadalonga <> $spadalonga ||$row->swordbuckler <> $swordbuckler ||
		...
		$rw_pollaxe,$rw_spear,$rw_armouredcombat,$rw_quarterstaff,$rw_archery,$rw_langesschwert,$rw_mountedcombat,$rapier);
...
Function PeopleDelete($LinkID, $Rec_ID) {
...
} // End Function PeopleDelete

Function RankDelete($LinkID, $Rec_ID) {
...
} // End Function RankDelete

Function ChapterDelete($LinkID, $Rec_ID) {
...
} // End Function ChapterDelete

Function MartialDelete($LinkID, $Rec_ID) {
...
} // End Function MartialDelete

Function MemberShow($LinkID, $State, $Rec_ID, $school, $uRec_ID, $login_ID, $seclvl) {
?>
	...
	<tr>
		...
		<td valign="center" id="Label" colspan="2" align="left">Birth Date (yyyy-mm-dd):&nbsp;<input type="text" name="BirthDate" maxlength="10" size="10" value="<?=$Line1->BirthDate?>" id="BirthDate">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td width="37%"><input type="text" name="EMail" maxlength="50" size="22" value="<?=$Line1->EMail?>"></td>
		<td valign="center" id="Label" colspan="2" style="vertical-align:middle;">Acad. Institution:&nbsp;<input type="text" name="Institution"  maxlength="64" size="20" value="<?=$Line1->AcademicInstitution?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td valign="center" colspan="3"><table border="0" align="left" width="100%" cellpadding="0" cellspacing="0" bgcolor="lightgrey" bordercolor="#000000">
			<tr>
				...
				<td valign="center" id="Label" align="left">Source:&nbsp;<input type="text"  name="Source" maxlength="64" size="40" value="<?=$Line1->ArmsSource?>" id="Source">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
			</tr></table></td>
	</tr>
	<tr>
	<?php
	...
	<tr>
		<td valign="top" colspan="2" id="Label">Transportation Mode:&nbsp;<select name="TransMode">
			...
			if ($State == "Create")
				{
				echo ('male:&nbsp;<input type="radio" name="Gender" value="M"  "CHECKED" style="vertical-align:middle;" >&nbsp;&nbsp;&nbsp;female:&nbsp;<input type="radio" name="Gender" value="F" style="vertical-align:middle;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>');
				}
			else
				{
				echo ('male:&nbsp;<input type="radio" name="Gender" value="M" '. ($Line1->Gender == 'M' ? 'CHECKED' : '') .' style="vertical-align:middle;" >&nbsp;&nbsp;&nbsp;female:&nbsp;<input type="radio" name="Gender" value="F" '. ($Line1->Gender == 'F' ? 'CHECKED' : '') .' style="vertical-align:middle;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>');
				}
			?>		
	</tr>
	<tr>
		<td valign="center" id="Label" colspan="2">First Aid Training?&nbsp;&nbsp;<input type="checkbox" name="FirstAid" value="1" style="vertical-align:middle;"  <?=$Line1->FirstAidTraining == "1" ? "CHECKED" : ""?>>&nbsp;&nbsp;</td>
	...
	<tr>
		<td valign="top"  colspan="2" id="Label">Previous Martial Arts Experience?&nbsp;&nbsp;<input type="checkbox" name="PreviousMartial" value="1" style="vertical-align:middle;"  <?=$Line1->PreviousMartial == "1" ? "CHECKED" : ""?>>&nbsp;&nbsp;</td>
	</tr>
	<tr>
		<td valign="top"  colspan="2" id="Label">Injured While Training @ AEMMA?&nbsp;&nbsp;<input type="checkbox" name="Injury" value="1" style="vertical-align:middle;"  <?=$Line1->Injury == "1" ? "CHECKED" : ""?>>&nbsp;&nbsp;</td>
	</tr>
<!-- the submit button occurs twice on this page, therefore, update one, must update the other -->
	<tr>
<?php
			echo ('<td colspan="4" align="center"><input type="hidden" name="School" value="'.$school.'"><input type="hidden" name="ID" value="'.$Rec_ID.'"><input type="hidden" name="State" value="Member'.$State.'"><input type="Submit" value="Save Membership Information">');
			if ($_SESSION["RoleID"] > 5) 
				{ echo ('<input type="Submit" value=" Delete This Record " name="State" style="background:#F778A1">'); }
?>
	</tr>
<!-- the submit button occurs twice on this page, therefore, update one, must update the other -->
	<tr>
...
?>
			...
			Mem. Category: <select name="MemType">
<?php
...
?>			</select></td>
			<td id="Label" colspan="2"><div align="left">&nbsp;&nbsp;&nbsp;Date Achieved: <input type="text" maxlength="10" size="10" value="<?=$Line4->Date?>" name="RankDate" id="RankDate"><input type="hidden" name="RankCurrent" value="<?=$Line4->Current?>">&nbsp;&nbsp;&nbsp;<br />
			<script type="text/javascript">
				Calendar.setup( {
					inputField : "RankDate",
					ifFormat   : "%Y-%m-%d"
...
if ($seclvl < 3) 
	...
	switch($State) 
		...
		case 'Create':
		case 'Update':
echo ('debug:about to go into membershow');
			MemberShow($LinkID, $State, $Rec_ID, $school, $uRec_ID, $login_ID, $seclvl);
			break;
		case 'MemberUpdate':
...
//				$Shield 		= "http://www.aemma.org/images/coatofarms/arms_aemma.jpg";
				...
				MemberCreate($LinkID, $Rec_ID, $_POST['School'], $_POST['FName'], $_POST['MName'], $_POST['LName'], $_POST['Salutation'], $_POST['Protocol'],
					...
					$Chapter);
				include 'function_LogAccess.php';
				$resource = "created record ID: $Rec_ID";
				LogAccess($LinkID, $uRec_ID, $login_ID, $seclvl, $resource); 
				}
			else
				{
			...
			case 'MemberDelete':
					...
					{
					PeopleDelete($LinkID, $Rec_ID);
					RankDelete($LinkID, $Rec_ID);
					ChapterDelete($LinkID, $Rec_ID);
					MartialDelete($LinkID, $Rec_ID);
					//HarnessDelete($LinkID, $Rec_ID);
					//InjuriesDelete($LinkID, $Rec_ID);
					//PortraitDelete($LinkID, $Rec_ID);
					}
				else
					{
					echo ('<script type="text/JavaScript">');
