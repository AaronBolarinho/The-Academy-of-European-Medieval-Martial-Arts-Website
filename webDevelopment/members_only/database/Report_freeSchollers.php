...
<?php
	...
	echo ('<caption>Membership Listing: Toronto Free Schollers  ('.$full_date.')</caption>');
?>
	...
	<?php
		...
		$SQL .= ' WHERE P.School = "AEMMA" and (P.ID = MC.People_ID and MC.Chapter_ID = 1 and P.Rank = 3) and (P.MemberStatus_ID = 2 or P.MemberStatus_ID = 1 or P.MemberStatus_ID = 6)';
		...
		$SQL .= ' FROM People P, Members_Chapter MC';
		$SQL .= ' WHERE P.School = "AEMMA" and (P.ID = MC.People_ID and MC.Chapter_ID = 1 and P.Rank = 3) and (P.MemberStatus_ID = 2 or P.MemberStatus_ID = 1)';
		$Result = mysql_query($SQL, $LinkID);
		while ($Line = mysql_fetch_object($Result)) {
...
<?php
	$full_date= date("d-m-Y");	
	echo ('<caption>Membership Listing: Guelph Free Schollers  ('.$full_date.')</caption>');
?>
		<tr><th>Name</th><th>City</th><th>Email</th><th>Telephone (H)</th><th>Photo&nbsp;</th></tr>
	<?php
		$LinkID=dbConnect($DB);
		...
		$SQL .= ' FROM People P, Members_Chapter MC';
		$SQL .= ' WHERE P.School = "AEMMA" and (P.ID = MC.People_ID and MC.Chapter_ID = 2 and P.Rank = 3) and (P.MemberStatus_ID = 2 or P.MemberStatus_ID = 1 or P.MemberStatus_ID = 6 or P.MemberStatus_ID = 3)';
		$SQL .= ' ORDER BY P.LastName';
		...
		$SQL = 'SELECT count(*) Count';
		$SQL .= ' FROM People P, Members_Chapter MC';
		$SQL .= ' WHERE P.School = "AEMMA" and (P.ID = MC.People_ID and MC.Chapter_ID = 2 and P.Rank = 3) and (P.MemberStatus_ID = 2 or P.MemberStatus_ID = 1)';
		$Result = mysql_query($SQL, $LinkID);
		while ($Line = mysql_fetch_object($Result)) {
			echo ('<tr bgcolor="gray"><td colspan=5><font face="arial,helvetica" size=2 color="white">&nbsp;<b>Total: </b>'.$Line->Count.'</td></tr><tr><td colspan="4">&nbsp;</td></tr>');
		}
	...
	?>
	<tr>
		<td colspan="4"><font face="arial,helvetica" size=2><b>Note: </b>Free Schollers whose a) membership type is identified as "any"; b) membership status is "any", and records highlighted in <font color="purple">"purple"</font> are inactive, records highlighted in <font color="blue">"blue"</font> are suspended due to work or health reasons.</font></td>
	</tr>
