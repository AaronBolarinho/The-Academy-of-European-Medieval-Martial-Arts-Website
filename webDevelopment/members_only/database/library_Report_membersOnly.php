...
//	$full_date = date("Y-m-d");	
	...
	echo ('<caption>AEMMA Students/Members Only Resources Listing&nbsp;('.$full_date.')</caption>');
	echo ('<tr><th>UserID</th><th>Name</th><th>Country</th><th>Email</th><th>Privileged</th><th>Access Status</th><th>Member Status</th><th>Member Access Count&nbsp;</th></tr>');

	$LinkID=dbConnect($DB);
	$SQL = 'SELECT R.ID, R.userID, R.first_name, R.middle_initial, R.surname, R.country, R.organization, R.email, R.privileged, R.access_status, R.member_status, R.member_access_counter';
	$SQL .= ' FROM registeredUsers R';
	$SQL .= ' WHERE R.member_status = 1';
	...
	while ($Line = mysql_fetch_object($Result)) {
		...
		if ($status == 0)
			...
			echo ('<td><div id="Data" align="center"><font color="red">' . $Line->member_status . '</font></div></td>');
			echo ('<td><div id="Data" align="center"><font color="red">' . $Line->member_access_counter . '</font></div></td>');
			}
		else
			{ 
			...
			echo ('<td><div id="Data" align="center">' . $Line->member_status . '</div></td>');
			echo ('<td><div id="Data" align="center">' . $Line->member_access_counter . '</div></td>');
			}
		echo ('</tr>');
	}
	...
	$Result = mysql_query($SQL, $LinkID);
	while ($Line = mysql_fetch_object($Result)) {
		echo ('<tr bgcolor="gray"><td colspan="8"><font face="arial,helvetica,sans-serif" size="-2" color="white">&nbsp;<b>Total number of Members Only: </b>'.$Line->Count.'&nbsp;&nbsp;</td></tr>');
	}
